...
use crate::{
	...
	sqlite::{
		...
		SynapseAuxiliaryRow, SynapseBackwardExtremity, SynapseCrossSigningKey, SynapseCurrentStateDelta, SynapseDevice, SynapseDeviceAuthProvider, SynapseDehydratedDevice,
		...
		SynapseEventPushSummary, SynapseEventPushSummaryLastReceiptStreamId,
		SynapseEventPushSummaryStreamPosition, SynapseEventRelation, SynapseEventReport,
		SynapseEventSearch, SynapseEventToStateGroup, SynapseEventTransaction, SynapseExOutlierStream,
		...
		SynapseRoomAlias, SynapseRoomDepth, SynapseRoomEvent, SynapseRoomKeyBackup,
		SynapseRoomKeyBackupVersion, SynapseRoomMetadata, SynapseRoomRetention,
		SynapseRoomState, SynapseRoomStatsCurrent,
		...
		SynapseUserDirectoryStreamPosition, SynapseUserExternalId, SynapseUserIp,
		SynapseUserSignatureStream, SynapseUserStatsCurrent, SynapseUsersInPublicRoom,
...
const REQUIRED_CFS: &[&str] = &[
	...
	"synapse_user_ips",
	"synapse_user_stats_current",
	"registrationtoken_info",
	"userid_displayname",
	"userid_avatarurl",
	"email_localpart",
	"localpart_email",
	"synapse_threepid_validation_sessions",
	"synapse_user_threepid_id_servers",
	...
	"synapse_received_transactions",
	"synapse_destinations",
	"synapse_destination_rooms",
	"synapse_event_failed_pull_attempts",
	"synapse_cache_invalidation_stream",
	"synapse_federation_stream_position",
	"synapse_federation_inbound_events_staging",
	...
	"synapse_rooms",
	"synapse_room_depth",
	"synapse_room_tags_revisions",
	"synapse_users_in_public_rooms",
	"synapse_users_who_share_private_rooms",
	"synapse_user_directory",
	"synapse_user_directory_search",
	"synapse_user_directory_stale_remote_users",
	"synapse_user_directory_stream_pos",
	"synapse_room_stats_current",
	"synapse_room_stats_state",
	"synapse_room_stats_earliest_token",
	"userroomid_joined",
	"roomuserid_joined",
	...
	"synapse_sliding_sync_connections",
	"synapse_sliding_sync_connection_positions",
	"synapse_sliding_sync_connection_streams",
	...
	"synapse_sliding_sync_membership_snapshots",
	"synapse_sliding_sync_joined_rooms",
	"synapse_sliding_sync_joined_rooms_to_recalculate",
	"synapse_stream_positions",
	"synapse_delayed_events_stream_pos",
...
pub struct ImportReport {
	...
	pub user_ips: u64,
	pub user_stats_current: u64,
	pub registration_tokens: u64,
	pub profiles: u64,
	pub threepids: u64,
	pub threepid_validation_sessions: u64,
	pub user_threepid_id_servers: u64,
	...
	pub destination_rooms: u64,
	pub event_failed_pull_attempts: u64,
	pub cache_invalidations: u64,
	pub federation_stream_positions: u64,
	pub federation_inbound_events: u64,
	...
	pub room_metadata: u64,
	pub room_depths: u64,
	pub users_in_public_rooms: u64,
	pub users_who_share_private_rooms: u64,
	pub user_directory_entries: u64,
	pub user_directory_search_entries: u64,
	pub user_directory_stale_remote_users: u64,
	pub user_directory_stream_positions: u64,
	pub room_stats_current: u64,
	pub room_stats_state: u64,
	pub room_stats_earliest_tokens: u64,
	pub receipts: u64,
	pub receipts_graph: u64,
	...
	pub sliding_sync_connections: u64,
	pub sliding_sync_connection_positions: u64,
	pub sliding_sync_connection_streams: u64,
	pub sliding_sync_connection_room_configs: u64,
	pub sliding_sync_connection_required_state: u64,
	pub sliding_sync_connection_lazy_members: u64,
	pub sliding_sync_membership_snapshots: u64,
	pub sliding_sync_joined_rooms: u64,
	pub sliding_sync_joined_rooms_to_recalculate: u64,
	pub stream_positions: u64,
	pub delayed_events_stream_positions: u64,
...
impl ContinuwuityStore {
	...
	pub fn import_user_activity(
		&self,
		ips: Vec<SynapseUserIp>,
		stats: Vec<SynapseUserStatsCurrent>,
		report: &mut ImportReport,
	) -> Result<()> {
		if !ips.is_empty() || !stats.is_empty() {
			report.warn(
				"Synapse user activity metadata was preserved for audit; continuwuity computes live activity and usage stats independently"
					.to_owned(),
			);
		}
		...
		for row in ips {
			if !row.user_id.starts_with('@')
				|| row.access_token.is_empty()
				|| row.ip.is_empty()
				|| row.user_agent.is_empty()
			{
				report.skip("user_ips.invalid");
				continue;
			}
			if row.last_seen < 0 {
				report.skip("user_ips.invalid_last_seen");
				continue;
			}

			let sequence = report.user_ips;
			...
			self.put_raw("synapse_user_ips", &key, &serde_json::to_vec(&value)?)?;
			report.user_ips = report.user_ips.saturating_add(1);
		}

		for row in stats {
			if !row.user_id.starts_with('@') {
				report.skip("user_stats_current.invalid_user_id");
				continue;
			}
			if row.joined_rooms < 0 || row.completed_delta_stream_id < 0 {
				report.skip("user_stats_current.invalid");
				continue;
			}
			...
			let value = json!({
				...
				"completed_delta_stream_id": row.completed_delta_stream_id,
			});
			self.put_raw(
				"synapse_user_stats_current",
				row.user_id.as_bytes(),
			...
			report.user_stats_current = report.user_stats_current.saturating_add(1);
		}
	...
	pub fn import_threepids(
		...
		validation_sessions: Vec<SynapseThreepidValidationSession>,
		id_servers: Vec<SynapseThreepidIdServer>,
		report: &mut ImportReport,
	) -> Result<()> {
		...
		if !validation_sessions.is_empty() {
			report.warn(
				"Synapse threepid validation sessions were preserved for audit; continuwuity does not replay pending validation sessions"
					.to_owned(),
			);
		}
		...
		for session in validation_sessions {
			if session.session_id.is_empty()
				|| session.medium.is_empty()
				|| session.address.is_empty()
				|| session.client_secret.is_empty()
			{
				report.skip("threepid_validation_sessions.invalid");
				continue;
			}
			if session.last_send_attempt < 0
				|| session.validated_at.is_some_and(|timestamp| timestamp < 0)
			{
				report.skip("threepid_validation_sessions.invalid_timestamp");
				continue;
			}
			...
			report.threepid_validation_sessions =
				report.threepid_validation_sessions.saturating_add(1);
		}

		if !id_servers.is_empty() {
			report.warn(
				"Synapse threepid identity-server bindings were preserved for audit; continuwuity does not contact legacy identity servers during migration"
					.to_owned(),
			);
		}

		for row in id_servers {
			if !row.user_id.starts_with('@')
				|| row.medium.is_empty()
				|| row.address.is_empty()
				|| row.id_server.is_empty()
			{
				report.skip("user_threepid_id_server.invalid");
				continue;
			}
			...
			report.user_threepid_id_servers =
				report.user_threepid_id_servers.saturating_add(1);
		}
	...
	pub fn import_federation_metadata(
		&mut self,
		received_transactions: Vec<SynapseReceivedTransaction>,
		destinations: Vec<SynapseDestination>,
		destination_rooms: Vec<SynapseDestinationRoom>,
		failed_pull_attempts: Vec<SynapseEventFailedPullAttempt>,
		cache_invalidations: Vec<SynapseCacheInvalidation>,
		stream_positions: Vec<SynapseFederationStreamPosition>,
		inbound_events: Vec<SynapseFederationInboundEvent>,
		report: &mut ImportReport,
	) -> Result<()> {
		if !received_transactions.is_empty()
			|| !destinations.is_empty()
			|| !destination_rooms.is_empty()
			|| !failed_pull_attempts.is_empty()
			|| !cache_invalidations.is_empty()
			|| !stream_positions.is_empty()
			|| !inbound_events.is_empty()
		{
			report.warn(
				"Synapse federation metadata was preserved for audit; continuwuity rebuilds live federation queues from imported room state and local activity"
					.to_owned(),
			);
		}

		for row in received_transactions {
			let sequence = report.received_transactions;
			let key = serialize_to_vec((
				row.origin.as_deref().unwrap_or_default(),
				row.transaction_id.as_deref().unwrap_or_default(),
				row.ts.unwrap_or_default(),
				sequence,
			))?;
			let value = json!({
				...
				"response_json_base64": row.response_json
					.as_ref()
					.map(|bytes| BASE64_STANDARD.encode(bytes)),
				"has_been_referenced": row.has_been_referenced,
			});
			self.put_raw(
				"synapse_received_transactions",
				&key,
			...
			report.received_transactions = report.received_transactions.saturating_add(1);
		}

		for row in destinations {
			if row.destination.is_empty() {
				report.skip("destinations.invalid");
				continue;
			}

			let value = json!({
				"destination": &row.destination,
				"retry_last_ts": row.retry_last_ts,
				"retry_interval": row.retry_interval,
				"failure_ts": row.failure_ts,
				"last_successful_stream_ordering": row.last_successful_stream_ordering,
			});
			self.put_raw(
				"synapse_destinations",
				row.destination.as_bytes(),
				&serde_json::to_vec(&value)?,
			)?;
			report.destinations = report.destinations.saturating_add(1);
		}

		for row in destination_rooms {
			let Some(stream_ordering) = u64::try_from(row.stream_ordering).ok() else {
				report.skip("destination_rooms.invalid_stream_ordering");
				continue;
			};
			if row.destination.is_empty() || !row.room_id.starts_with('!') {
				report.skip("destination_rooms.invalid");
				continue;
			}

			let key = serialize_to_vec((&row.destination, &row.room_id))?;
			let value = json!({
				"destination": &row.destination,
				"room_id": &row.room_id,
			...
			self.reserve_count(stream_ordering)?;
			report.destination_rooms = report.destination_rooms.saturating_add(1);
		}

		for row in failed_pull_attempts {
			if !row.room_id.starts_with('!')
				|| !row.event_id.starts_with('$')
				|| row.num_attempts < 0
				|| row.last_attempt_ts < 0
			{
				report.skip("event_failed_pull_attempts.invalid");
				continue;
			}
			...
			report.event_failed_pull_attempts =
				report.event_failed_pull_attempts.saturating_add(1);
		}

		for row in cache_invalidations {
			let Some(stream_id) = u64::try_from(row.stream_id).ok() else {
				report.skip("cache_invalidations.invalid_stream_id");
				continue;
			};
			if row.instance_name.is_empty() || row.cache_func.is_empty() {
				report.skip("cache_invalidations.invalid");
				continue;
			}

			let key = serialize_to_vec((stream_id, &row.instance_name))?;
			let value = json!({
				"stream_id": row.stream_id,
				"instance_name": &row.instance_name,
				"cache_func": &row.cache_func,
				"keys": &row.keys,
				"invalidation_ts": row.invalidation_ts,
			});
			self.put_raw(
				"synapse_cache_invalidation_stream",
				&key,
			...
			report.cache_invalidations = report.cache_invalidations.saturating_add(1);
		}

		for row in stream_positions {
			if row.stream_type.is_empty() {
				report.skip("federation_stream_positions.invalid");
				continue;
			}

			let key = serialize_to_vec((
				&row.stream_type,
				row.instance_name.as_deref().unwrap_or_default(),
			))?;
			let value = json!({
			...
			report.federation_stream_positions =
				report.federation_stream_positions.saturating_add(1);
		}
		...
		for row in inbound_events {
			if row.origin.is_empty()
				...
				|| !row.event_id.starts_with('$')
				|| row.received_ts < 0
				|| !row.event_json.is_object()
				|| !row.internal_metadata.is_object()
			{
				report.skip("federation_inbound_events.invalid");
				continue;
			}

			let key = serialize_to_vec((&row.origin, &row.event_id))?;
			let value = json!({
				"origin": &row.origin,
				"room_id": &row.room_id,
				"event_id": &row.event_id,
			...
			report.federation_inbound_events =
				report.federation_inbound_events.saturating_add(1);
		}
	...
	pub fn import_room_tags(
		...
		revisions: Vec<SynapseRoomTagRevision>,
		report: &mut ImportReport,
	) -> Result<()> {
		...
		if !revisions.is_empty() {
			report.warn(
				"Synapse room tag revision metadata was preserved for audit; continuwuity assigns new account-data stream positions after import"
					.to_owned(),
			);
		}

		for revision in revisions {
			if !revision.user_id.starts_with('@')
				|| !revision.room_id.starts_with('!')
				|| revision.stream_id < 0
			{
				report.skip("room_tags_revisions.invalid");
				continue;
			}

			let key = serialize_to_vec((&revision.user_id, &revision.room_id))?;
			let value = json!({
				"user_id": &revision.user_id,
				"room_id": &revision.room_id,
				"stream_id": revision.stream_id,
				"instance_name": &revision.instance_name,
			});
			self.put_raw(
				"synapse_room_tags_revisions",
				&key,
			...
			report.room_tag_revisions = report.room_tag_revisions.saturating_add(1);
		}
	...
	pub fn import_event_search(
		&self,
		rows: Vec<SynapseEventSearch>,
		report: &mut ImportReport,
	) -> Result<()> {
		if !rows.is_empty()
			&& !report
		...
		{
			report.warn(
				"Synapse event_search rows were preserved for audit; continuwuity rebuilds its runtime search index from imported events"
					.to_owned(),
			);
		}

		for row in rows {
			let Some(event_id) = row.event_id.as_deref().filter(|event_id| event_id.starts_with('$'))
			else {
				report.skip("event_search.invalid_event_id");
				continue;
			};
			if row
				.room_id
				...
				.is_some_and(|room_id| !room_id.starts_with('!'))
			{
				report.skip("event_search.invalid_room_id");
				continue;
			}
			if row
				.sender
			...
			{
				report.skip("event_search.invalid_sender");
				continue;
			}
			if row
				.origin_server_ts
				.is_some_and(|origin_server_ts| origin_server_ts < 0)
			{
				report.skip("event_search.invalid_origin_server_ts");
				continue;
			}
			if row
				.stream_ordering
				.is_some_and(|stream_ordering| stream_ordering < 0)
			{
				report.skip("event_search.invalid_stream_ordering");
				continue;
			}
			...
			report.synapse_event_search = report.synapse_event_search.saturating_add(1);
		}
	...
	pub fn import_state_events(
		&self,
		rows: Vec<SynapseStateEvent>,
		report: &mut ImportReport,
	) -> Result<()> {
		if !rows.is_empty()
			&& !report
		...
		{
			report.warn(
				"Synapse state_events table metadata was preserved for audit; continuwuity rebuilds runtime state from imported current state and state groups"
					.to_owned(),
			);
		}
		...
		for row in rows {
			if !row.event_id.starts_with('$') {
				report.skip("state_events.invalid_event_id");
				continue;
			}
			if !row.room_id.starts_with('!') || row.event_type.is_empty() {
				report.skip("state_events.invalid");
				continue;
			}
			if row
				.prev_state
				.as_deref()
				.is_some_and(|prev_state| !prev_state.starts_with('$'))
			{
				report.skip("state_events.invalid_prev_state");
				continue;
			}
			...
			report.state_events = report.state_events.saturating_add(1);
		}

		Ok(())
	}

	pub fn import_state_stream_metadata(
		&self,
		deltas: Vec<SynapseCurrentStateDelta>,
		extremities: Vec<SynapseStreamOrderingExtremity>,
		outliers: Vec<SynapseExOutlierStream>,
		report: &mut ImportReport,
	) -> Result<()> {
		if (!deltas.is_empty() || !extremities.is_empty() || !outliers.is_empty())
			&& !report.warnings.iter().any(|warning| warning.contains("state stream metadata was preserved"))
		{
			report.warn(
				"Synapse state stream metadata was preserved for audit; continuwuity rebuilds runtime sync/state streams from imported room state and events"
					.to_owned(),
			);
		}
		...
		for row in deltas {
			let Some(stream_id) = u64::try_from(row.stream_id).ok() else {
				report.skip("current_state_delta_stream.invalid_stream_id");
				continue;
			};
			if !row.room_id.starts_with('!')
				...
				|| row
					...
					.is_some_and(|event_id| !event_id.starts_with('$'))
			{
				report.skip("current_state_delta_stream.invalid");
				continue;
			}
			...
			report.current_state_delta_stream = report.current_state_delta_stream.saturating_add(1);
		}

		for row in extremities {
			let Some(stream_ordering) = u64::try_from(row.stream_ordering).ok() else {
				report.skip("stream_ordering_to_extremity.invalid_stream_ordering");
				continue;
			};
			if !row.room_id.starts_with('!') || !row.event_id.starts_with('$') {
				report.skip("stream_ordering_to_extremity.invalid");
				continue;
			}

			let key = serialize_to_vec((stream_ordering, &row.room_id, &row.event_id))?;
			let value = json!({
				"stream_ordering": row.stream_ordering,
				"room_id": &row.room_id,
				"event_id": &row.event_id,
			...
			report.stream_ordering_to_extremity = report.stream_ordering_to_extremity.saturating_add(1);
		}

		for row in outliers {
			let Some(event_stream_ordering) = u64::try_from(row.event_stream_ordering).ok() else {
				report.skip("ex_outlier_stream.invalid_stream_ordering");
				continue;
			};
			if !row.event_id.starts_with('$') {
				report.skip("ex_outlier_stream.invalid");
				continue;
			}
			if row.state_group < 0 {
				report.skip("ex_outlier_stream.invalid_state_group");
				continue;
			}
			...
			self.put_raw(
				"synapse_ex_outlier_stream",
				&event_stream_ordering.to_be_bytes(),
				&serde_json::to_vec(&value)?,
			)?;
			report.ex_outlier_stream = report.ex_outlier_stream.saturating_add(1);
		}
	...
	pub fn import_receipts(
		...
		graph_receipts: Vec<SynapseReceiptGraph>,
		report: &mut ImportReport,
	) -> Result<()> {
		...
		if !graph_receipts.is_empty() {
			report.warn(
				"Synapse graph receipt metadata was preserved for audit; continuwuity imports active receipt positions from linearized receipts"
					.to_owned(),
			);
		}
		...
		for receipt in graph_receipts {
			if !receipt.room_id.starts_with('!')
				|| receipt.receipt_type.is_empty()
				|| !receipt.user_id.starts_with('@')
				|| receipt.event_ids.is_empty()
				|| receipt.event_ids.iter().any(|event_id| !event_id.starts_with('$'))
				|| receipt.data.is_null()
			{
				report.skip("receipts_graph.invalid");
				continue;
			}
			...
			report.receipts_graph = report.receipts_graph.saturating_add(1);
		}
	...
	pub fn import_sliding_sync(
		&self,
		connections: Vec<SynapseSlidingSyncConnection>,
		positions: Vec<SynapseSlidingSyncConnectionPosition>,
		streams: Vec<SynapseSlidingSyncConnectionStream>,
		room_configs: Vec<SynapseSlidingSyncConnectionRoomConfig>,
		required_state: Vec<SynapseSlidingSyncConnectionRequiredState>,
		lazy_members: Vec<SynapseSlidingSyncConnectionLazyMember>,
		membership_snapshots: Vec<SynapseSlidingSyncMembershipSnapshot>,
		joined_rooms: Vec<SynapseSlidingSyncJoinedRoom>,
		joined_rooms_to_recalculate: Vec<SynapseSlidingSyncJoinedRoomToRecalculate>,
		report: &mut ImportReport,
	) -> Result<()> {
		if !connections.is_empty()
			|| !positions.is_empty()
			|| !streams.is_empty()
			|| !room_configs.is_empty()
			|| !required_state.is_empty()
			|| !lazy_members.is_empty()
			|| !membership_snapshots.is_empty()
			|| !joined_rooms.is_empty()
			|| !joined_rooms_to_recalculate.is_empty()
		{
			report.warn(
				"Synapse sliding-sync cache metadata was preserved for audit; continuwuity rebuilds live sliding-sync connections from client sync requests"
					.to_owned(),
			);
		}

		for row in connections {
			let Some(connection_key) = u64::try_from(row.connection_key).ok() else {
				report.skip("sliding_sync_connections.invalid_connection_key");
				continue;
			};
			if !row.user_id.starts_with('@')
				|| row.effective_device_id.is_empty()
				|| row.conn_id.is_empty()
				|| row.created_ts < 0
				|| row.last_used_ts.is_some_and(|value| value < 0)
			{
				report.skip("sliding_sync_connections.invalid");
				continue;
			}
			...
			let value = json!({
				...
				"user_id": &row.user_id,
				"effective_device_id": &row.effective_device_id,
				"conn_id": &row.conn_id,
			...
			self.put_raw(
				"synapse_sliding_sync_connections",
				&connection_key.to_be_bytes(),
				&serde_json::to_vec(&value)?,
			)?;
			report.sliding_sync_connections =
				report.sliding_sync_connections.saturating_add(1);
		}

		for row in positions {
			let Some(connection_position) = u64::try_from(row.connection_position).ok() else {
				report.skip("sliding_sync_connection_positions.invalid_position");
				continue;
			};
			if row.connection_key < 0 || row.created_ts < 0 {
				report.skip("sliding_sync_connection_positions.invalid");
				continue;
			}

			let value = json!({
				"connection_position": row.connection_position,
				"connection_key": row.connection_key,
				"created_ts": row.created_ts,
			});
			self.put_raw(
				"synapse_sliding_sync_connection_positions",
				&connection_position.to_be_bytes(),
				&serde_json::to_vec(&value)?,
			)?;
			report.sliding_sync_connection_positions =
				report.sliding_sync_connection_positions.saturating_add(1);
		}

		for row in streams {
			let Some(connection_position) = u64::try_from(row.connection_position).ok() else {
				report.skip("sliding_sync_connection_streams.invalid_position");
				continue;
			};
			if row.stream.is_empty() || !row.room_id.starts_with('!') || row.room_status.is_empty()
			{
				report.skip("sliding_sync_connection_streams.invalid");
				continue;
			}

			let key = serialize_to_vec((connection_position, &row.stream, &row.room_id))?;
			let value = json!({
				"connection_position": row.connection_position,
				"stream": &row.stream,
				"room_id": &row.room_id,
			...
			report.sliding_sync_connection_streams =
				report.sliding_sync_connection_streams.saturating_add(1);
		}

		for row in room_configs {
			let Some(connection_position) = u64::try_from(row.connection_position).ok() else {
				report.skip("sliding_sync_connection_room_configs.invalid_position");
				continue;
			};
			if !row.room_id.starts_with('!')
				|| row.timeline_limit < 0
				|| row.required_state_id < 0
			{
				report.skip("sliding_sync_connection_room_configs.invalid");
				continue;
			}

			let key = serialize_to_vec((connection_position, &row.room_id))?;
			let value = json!({
				"connection_position": row.connection_position,
				"room_id": &row.room_id,
				"timeline_limit": row.timeline_limit,
				"required_state_id": row.required_state_id,
			});
			self.put_raw(
				"synapse_sliding_sync_connection_room_configs",
				&key,
			...
			report.sliding_sync_connection_room_configs =
				report.sliding_sync_connection_room_configs.saturating_add(1);
		}

		for row in required_state {
			let Some(required_state_id) = u64::try_from(row.required_state_id).ok() else {
				report.skip("sliding_sync_connection_required_state.invalid_id");
				continue;
			};
			if row.connection_key < 0 || !row.required_state.is_array() {
				report.skip("sliding_sync_connection_required_state.invalid");
				continue;
			}
			...
			self.put_raw(
				"synapse_sliding_sync_connection_required_state",
				&required_state_id.to_be_bytes(),
				&serde_json::to_vec(&value)?,
			)?;
			report.sliding_sync_connection_required_state =
				report.sliding_sync_connection_required_state.saturating_add(1);
		}

		for row in lazy_members {
			let Some(connection_key) = u64::try_from(row.connection_key).ok() else {
				report.skip("sliding_sync_connection_lazy_members.invalid_connection_key");
				continue;
			};
			if row.connection_position.is_some_and(|value| value < 0)
				|| !row.room_id.starts_with('!')
				|| !row.user_id.starts_with('@')
				|| row.last_seen_ts < 0
			{
				report.skip("sliding_sync_connection_lazy_members.invalid");
				continue;
			}

			let key = serialize_to_vec((connection_key, &row.room_id, &row.user_id))?;
			let value = json!({
				"connection_key": row.connection_key,
				"connection_position": row.connection_position,
				"room_id": &row.room_id,
			...
			report.sliding_sync_connection_lazy_members =
				report.sliding_sync_connection_lazy_members.saturating_add(1);
		}

		for row in membership_snapshots {
			let Some(event_stream_ordering) = u64::try_from(row.event_stream_ordering).ok() else {
				report.skip("sliding_sync_membership_snapshots.invalid_stream_ordering");
				continue;
			};
			if !row.room_id.starts_with('!')
				|| !row.user_id.starts_with('@')
				|| !row.sender.starts_with('@')
				|| !row.membership_event_id.starts_with('$')
				|| row.membership.is_empty()
				|| row.event_instance_name.is_empty()
			{
				report.skip("sliding_sync_membership_snapshots.invalid");
				continue;
			}

			let key = serialize_to_vec((&row.room_id, &row.user_id, event_stream_ordering))?;
			let value = json!({
				"room_id": &row.room_id,
				"user_id": &row.user_id,
				"sender": &row.sender,
				"membership_event_id": &row.membership_event_id,
				"membership": &row.membership,
				"forgotten": row.forgotten,
				"event_stream_ordering": row.event_stream_ordering,
				"event_instance_name": &row.event_instance_name,
				"has_known_state": row.has_known_state,
				"room_type": &row.room_type,
				"room_name": &row.room_name,
				"is_encrypted": row.is_encrypted,
				"tombstone_successor_room_id": &row.tombstone_successor_room_id,
			});
			self.put_raw(
				"synapse_sliding_sync_membership_snapshots",
				&key,
			...
			report.sliding_sync_membership_snapshots =
				report.sliding_sync_membership_snapshots.saturating_add(1);
		}

		for row in joined_rooms {
			if !row.room_id.starts_with('!')
				|| row.event_stream_ordering < 0
				|| row.bump_stamp.is_some_and(|value| value < 0)
			{
				report.skip("sliding_sync_joined_rooms.invalid");
				continue;
			}
			...
			self.put_raw(
				"synapse_sliding_sync_joined_rooms",
				row.room_id.as_bytes(),
				&serde_json::to_vec(&value)?,
			)?;
			report.sliding_sync_joined_rooms =
				report.sliding_sync_joined_rooms.saturating_add(1);
		}

		for row in joined_rooms_to_recalculate {
			if !row.room_id.starts_with('!') {
				report.skip("sliding_sync_joined_rooms_to_recalculate.invalid");
				continue;
			}
			...
			report.sliding_sync_joined_rooms_to_recalculate =
				report.sliding_sync_joined_rooms_to_recalculate.saturating_add(1);
		}

		Ok(())
	}

	pub fn import_stream_positions(
		&self,
		stream_positions: Vec<SynapseStreamPosition>,
		delayed_events: Vec<SynapseDelayedEventsStreamPosition>,
		event_push_last_receipts: Vec<SynapseEventPushSummaryLastReceiptStreamId>,
		room_forgetter: Vec<SynapseRoomForgetterStreamPosition>,
		stats_incremental: Vec<SynapseStatsIncrementalPosition>,
		report: &mut ImportReport,
	) -> Result<()> {
		if !stream_positions.is_empty()
			|| !delayed_events.is_empty()
			|| !event_push_last_receipts.is_empty()
			|| !room_forgetter.is_empty()
			|| !stats_incremental.is_empty()
		{
			report.warn(
				"Synapse stream cursor metadata was preserved for audit; continuwuity computes runtime stream positions from imported data and current activity"
					.to_owned(),
			);
		}

		for row in stream_positions {
			if row.stream_name.is_empty() || row.instance_name.is_empty() {
				report.skip("stream_positions.invalid");
				continue;
			}

			let key = serialize_to_vec((&row.stream_name, &row.instance_name))?;
			let value = json!({
			...
			report.stream_positions = report.stream_positions.saturating_add(1);
		}

		for row in delayed_events {
			if row.lock.is_empty() {
				report.skip("delayed_events_stream_pos.invalid");
				continue;
			}
			...
			report.delayed_events_stream_positions =
				report.delayed_events_stream_positions.saturating_add(1);
		}

		for row in event_push_last_receipts {
			if row.lock.is_empty() {
				report.skip("event_push_summary_last_receipt_stream_id.invalid");
				continue;
			}
			...
			report.event_push_summary_last_receipt_stream_ids = report
				.event_push_summary_last_receipt_stream_ids
				.saturating_add(1);
		}

		for row in room_forgetter {
			if row.lock.is_empty() {
				report.skip("room_forgetter_stream_pos.invalid");
				continue;
			}
			...
			report.room_forgetter_stream_positions =
				report.room_forgetter_stream_positions.saturating_add(1);
		}

		for row in stats_incremental {
			if row.lock.is_empty() {
				report.skip("stats_incremental_position.invalid");
				continue;
			}
			...
			report.stats_incremental_positions =
				report.stats_incremental_positions.saturating_add(1);
		}
	...
	pub fn import_auxiliary_metadata(
		&self,
		rows: Vec<SynapseAuxiliaryRow>,
		report: &mut ImportReport,
	) -> Result<()> {
		if !rows.is_empty()
			&& !report
		...
		{
			report.warn(
				"Auxiliary Synapse metadata was preserved for audit; continuwuity either rebuilds equivalent runtime state or does not expose a direct runtime analogue yet"
					.to_owned(),
			);
		}
		...
		for row in rows {
			if row.table.is_empty() {
				report.skip("auxiliary_metadata.invalid_table");
				continue;
			}

			let key = serialize_to_vec((&row.table, row.ordinal))?;
			let value = json!({
				"table": &row.table,
				"ordinal": row.ordinal,
				"values": &row.values,
			...
			report.auxiliary_metadata = report.auxiliary_metadata.saturating_add(1);
		}
	...
	pub fn import_room_metadata(
		&self,
		rooms: Vec<SynapseRoomMetadata>,
		depths: Vec<SynapseRoomDepth>,
		report: &mut ImportReport,
	) -> Result<()> {
		if !rooms.is_empty() || !depths.is_empty() {
			report.warn(
				"Synapse room metadata was preserved for audit; continuwuity derives live room state from imported events and state groups"
					.to_owned(),
			);
		}
		...
		for room in rooms {
			if !room.room_id.starts_with('!') {
				report.skip("room_metadata.invalid_room_id");
				continue;
			}
			...
			let value = json!({
				...
				"has_auth_chain_index": room.has_auth_chain_index,
			});
			self.put_raw(
				"synapse_rooms",
				room.room_id.as_bytes(),
			...
			report.room_metadata = report.room_metadata.saturating_add(1);
		}

		for depth in depths {
			if !depth.room_id.starts_with('!') {
				report.skip("room_depth.invalid_room_id");
				continue;
			}

			let value = json!({
				"room_id": &depth.room_id,
				"min_depth": depth.min_depth,
			});
			self.put_raw(
				"synapse_room_depth",
				depth.room_id.as_bytes(),
				&serde_json::to_vec(&value)?,
			)?;
			report.room_depths = report.room_depths.saturating_add(1);
		}

		Ok(())
	}

	pub fn import_user_directory_metadata(
		&self,
		users_in_public_rooms: Vec<SynapseUsersInPublicRoom>,
		users_who_share_private_rooms: Vec<SynapseUsersWhoSharePrivateRoom>,
		user_directory: Vec<SynapseUserDirectoryEntry>,
		user_directory_search: Vec<SynapseUserDirectorySearch>,
		stale_remote_users: Vec<SynapseUserDirectoryStaleRemoteUser>,
		stream_positions: Vec<SynapseUserDirectoryStreamPosition>,
		room_stats_current: Vec<SynapseRoomStatsCurrent>,
		room_stats_state: Vec<SynapseRoomStatsState>,
		room_stats_earliest_tokens: Vec<SynapseRoomStatsEarliestToken>,
		report: &mut ImportReport,
	) -> Result<()> {
		if (!users_in_public_rooms.is_empty()
			|| !users_who_share_private_rooms.is_empty()
			|| !user_directory.is_empty()
			|| !user_directory_search.is_empty()
			|| !stale_remote_users.is_empty()
			|| !stream_positions.is_empty()
			|| !room_stats_current.is_empty()
			|| !room_stats_state.is_empty()
			|| !room_stats_earliest_tokens.is_empty())
			&& !report
				.warnings
				.iter()
		...
		{
			report.warn(
				"Synapse user-directory metadata was preserved for audit; continuwuity rebuilds runtime user search and public room summaries from imported profiles, events, and current state"
					.to_owned(),
			);
		}

		for row in users_in_public_rooms {
			if !row.user_id.starts_with('@') || !row.room_id.starts_with('!') {
				report.skip("users_in_public_rooms.invalid");
				continue;
			}
			...
			report.users_in_public_rooms = report.users_in_public_rooms.saturating_add(1);
		}

		for row in users_who_share_private_rooms {
			if !row.user_id.starts_with('@')
				|| !row.other_user_id.starts_with('@')
				|| !row.room_id.starts_with('!')
			{
				report.skip("users_who_share_private_rooms.invalid");
				continue;
			}

			let key = serialize_to_vec((&row.user_id, &row.other_user_id, &row.room_id))?;
			let value = json!({
				"user_id": &row.user_id,
			...
			report.users_who_share_private_rooms =
				report.users_who_share_private_rooms.saturating_add(1);
		}
		...
		for row in user_directory {
			if !row.user_id.starts_with('@')
				|| row
					...
					.is_some_and(|room_id| !room_id.starts_with('!'))
			{
				report.skip("user_directory.invalid");
				continue;
			}
			...
			report.user_directory_entries = report.user_directory_entries.saturating_add(1);
		}

		for row in user_directory_search {
			if !row.user_id.starts_with('@') {
				report.skip("user_directory_search.invalid");
				continue;
			}
			...
			report.user_directory_search_entries =
				report.user_directory_search_entries.saturating_add(1);
		}

		for row in stale_remote_users {
			if !row.user_id.starts_with('@')
				|| row.user_server_name.is_empty()
				|| row.next_try_at_ts < 0
				|| row.retry_counter < 0
			{
				report.skip("user_directory_stale_remote_users.invalid");
				continue;
			}

			let value = json!({
				"user_id": &row.user_id,
				"user_server_name": &row.user_server_name,
				"next_try_at_ts": row.next_try_at_ts,
				"retry_counter": row.retry_counter,
			});
			self.put_raw(
				"synapse_user_directory_stale_remote_users",
				row.user_id.as_bytes(),
			...
			report.user_directory_stale_remote_users =
				report.user_directory_stale_remote_users.saturating_add(1);
		}

		for row in stream_positions {
			if row.lock.is_empty() || row.stream_id.is_some_and(|stream_id| stream_id < 0) {
				report.skip("user_directory_stream_pos.invalid");
				continue;
			}
			...
			report.user_directory_stream_positions =
				report.user_directory_stream_positions.saturating_add(1);
		}

		for row in room_stats_current {
			if !row.room_id.starts_with('!')
				|| row.current_state_events < 0
				|| row.joined_members < 0
				|| row.invited_members < 0
				|| row.left_members < 0
				|| row.banned_members < 0
				|| row.local_users_in_room < 0
				|| row.completed_delta_stream_id < 0
				|| row.knocked_members.is_some_and(|value| value < 0)
			{
				report.skip("room_stats_current.invalid");
				continue;
			}

			let value = json!({
				"room_id": &row.room_id,
				"current_state_events": row.current_state_events,
				"joined_members": row.joined_members,
				"invited_members": row.invited_members,
				"left_members": row.left_members,
				"banned_members": row.banned_members,
				"local_users_in_room": row.local_users_in_room,
				"completed_delta_stream_id": row.completed_delta_stream_id,
				"knocked_members": row.knocked_members,
			});
			self.put_raw(
				"synapse_room_stats_current",
				row.room_id.as_bytes(),
			...
			report.room_stats_current = report.room_stats_current.saturating_add(1);
		}

		for row in room_stats_state {
			if !row.room_id.starts_with('!') {
				report.skip("room_stats_state.invalid");
				continue;
			}
			...
			let value = json!({
				...
				"is_federatable": row.is_federatable,
				"topic": &row.topic,
				"room_type": &row.room_type,
			});
			self.put_raw(
			...
			report.room_stats_state = report.room_stats_state.saturating_add(1);
		}

		for row in room_stats_earliest_tokens {
			if !row.room_id.starts_with('!') || row.token < 0 {
				report.skip("room_stats_earliest_token.invalid");
				continue;
			}
			...
			report.room_stats_earliest_tokens =
				report.room_stats_earliest_tokens.saturating_add(1);
		}
...
impl ImportReport {
	...
	pub fn to_text(&self) -> String {
		format!(
			...
			self.user_ips,
			self.user_stats_current,
			self.registration_tokens,
			self.profiles,
			self.threepids,
			self.threepid_validation_sessions,
			self.user_threepid_id_servers,
			...
			self.event_failed_pull_attempts,
			self.cache_invalidations,
			self.federation_stream_positions,
			self.federation_inbound_events,
			...
			self.room_metadata,
			self.room_depths,
			self.users_in_public_rooms,
			self.users_who_share_private_rooms,
			self.user_directory_entries,
			self.user_directory_search_entries,
			self.user_directory_stale_remote_users,
			self.user_directory_stream_positions,
			self.room_stats_current,
			self.room_stats_state,
			...
			self.sliding_sync_connections,
			self.sliding_sync_connection_positions,
			self.sliding_sync_connection_streams,
			self.sliding_sync_connection_room_configs,
			self.sliding_sync_connection_required_state,
			self.sliding_sync_connection_lazy_members,
			self.sliding_sync_membership_snapshots,
			self.sliding_sync_joined_rooms,
			self.sliding_sync_joined_rooms_to_recalculate,
			self.stream_positions,
			self.delayed_events_stream_positions,
